...
/**
 * Rota para obter estatísticas de discagem agrupadas por hora do dia para o gráfico de barras
 */
app.get('/api/dashboard/hourly-stats', (req, res) => {
  const { campaignId, startHour = 8, endHour = 21 } = req.query;
  try {
    let whereClause = "WHERE (call_status IN ('completed', 'failed') OR sms_status = 'completed')";
    const params = [];
    ...
    const query = `
      SELECT 
        strftime('%H', COALESCE(updated_at, created_at)) as hour,
        occurrence,
        call_status,
        sms_status,
        COUNT(id) as count
      FROM leads
      ${whereClause}
      GROUP BY hour, occurrence, call_status, sms_status
    `;

    const rawRows = all(query, params);

    const hoursMap = {};
    const minH = parseInt(startHour, 10) || 8;
    const maxH = parseInt(endHour, 10) || 21;
    ...
    rawRows.forEach(row => {
      ...
      const occ = String(row.occurrence || '').toUpperCase();

      if (occ.includes('ATENDEU') || occ.includes('CONFIRMOU') || occ.includes('ENVIO SMS') || row.call_status === 'completed' || row.sms_status === 'completed') {
        hoursMap[hStr].atendeu += count;
      } else if (occ.includes('3 DIAS') || occ.includes('QUARENTENA')) {
        hoursMap[hStr].quarentena3Dias += count;
      } else {
        hoursMap[hStr].naoAtendeu += count;
      }
      hoursMap[hStr].total += count;
    });

    const hourlyData = Object.keys(hoursMap).sort().map(k => hoursMap[k]);
    res.json(hourlyData);
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
